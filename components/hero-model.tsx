...
function PremiumAstronaut(props: ThreeElements["group"]) {
  ...
  const scale = isMobile ? 1.08 : 1.38
  ...
  return (
    <group ref={group} {...props} scale={[scale, scale, scale]} rotation={[0.04, -0.12, 0]}>
      <Float speed={1.12} rotationIntensity={0.12} floatIntensity={0.18} floatingRange={[-0.08, 0.12]}>
...
export default function HeroModel() {
  return (
    <div className="h-full w-full">
      <Canvas
        shadows
        camera={{ position: [0, 0.08, 10.4], fov: 34 }}
        dpr={[1, 1.5]}
      ...
      >
        ...
        <PremiumAstronaut position={[0, 0.16, 0]} />
        <Environment preset="night" />
        <ContactShadows position={[0, -2.42, 0]} opacity={0.24} scale={8} blur={1.5} far={4} frames={1} />
        <OrbitControls
          enableZoom={false}
          enablePan={false}
          enableDamping
